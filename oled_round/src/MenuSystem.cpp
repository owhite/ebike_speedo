...
const uint8_t MenuComponent::get_type() const {
  return _type;
}
...
void MenuComponent::set_type(const uint8_t* type) {
  _type = type;
}
...
void Menu::add_menu(Menu* p_menu, uint8_t type) {
  add_component((MenuComponent*) p_menu);
  p_menu->set_parent(this);
  p_menu->set_type(type);
}
...
uint32_t MenuSystem::get_last_menu_time() {
